...
train_data_path = r'data\water\water.csv'  # 학습 데이터 파일 경로
new_data_path = r'data\_weather_info\weekends_only.csv'  # 예측에 사용할 새로운 데이터 파일 경로
model_save_path =r'model\water\new_gru_water.pth'  # 학습된 GRU 모델 가중치 파일 경로
arima_model_save_path = r'model\water\new_arima_model_water.pkl'  # 학습된 ARIMA 모델 파일 경로
...
with open(r'data\water\water_forecasting.txt', 'w') as f:
