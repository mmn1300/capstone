...
train_data_path = r'data\icecream\icecream.csv'  # 학습 데이터 파일 경로
new_data_path = r'data\_weather_info\weekends_only.csv'  # 예측에 사용할 새로운 데이터 파일 경로
model_save_path =r'model\icecream\new_gru_icecream.pth'  # 학습된 GRU 모델 가중치 파일 경로
arima_model_save_path = r'model\icecream\new_arima_model_icecream.pkl'  # 학습된 ARIMA 모델 파일 경로
...
features_train = train_data.drop(columns=['measurementdate', 'sales_rate','windspeed_max','windspeed_avg','precipitation'])
...
features_new = new_data.drop(columns=['measurementdate', 'sales_rate','windspeed_max','windspeed_avg','precipitation'])
...
with open(r'data\icecream\icecream_forecasting.txt', 'w') as f:
