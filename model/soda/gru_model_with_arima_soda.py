...
train_data_path = r'data\soda\soda.csv'  # 학습 데이터 파일 경로
new_data_path = r'data\_weather_info\weekends_only.csv'  # 예측에 사용할 새로운 데이터 파일 경로
model_save_path =r'model\soda\new_gru_drink.pth'  # 학습된 GRU 모델 가중치 파일 경로
arima_model_save_path = r'model\soda\new_arima_model_drink.pkl'  # 학습된 ARIMA 모델 파일 경로
...
with open(r'data\soda\soda_forecasting.txt', 'w') as f:
